...
base_bert_file = './chinese_L-12_H-768_A-12/'    #中文预训练模型
base_file = ''
data_dir = './data/'  #存放输入数据集的文件夹
path_output_time = './output/' #str(time.time())  #训练好的分类器模型的存放文件夹
...
#bert中文模型中的vocab.txt文件
flags.DEFINE_string("vocab_file", base_bert_file + 'vocab.txt',
                    "The vocabulary file that the BERT model was trained on.")

#训练好的分类器模型的存放文件夹
flags.DEFINE_string(
    "output_dir", path_output_time,
    "The output directory where the model checkpoints will be written."
)

## Other parameters
#bert中文模型中的bert_model.ckpt.index文件
flags.DEFINE_string(
...
#输入文本序列的最大长度，也就是每个样本的最大处理长度，多余会去掉，不够会补齐。最大值512
...
#        raise ValueError("At least one of `do_train` or `do_eval` must be True.")
    ...
    if FLAGS.do_predict:
        ...
        predict_file = os.path.join(FLAGS.output_dir, "result_dir/predict.tf_record")
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, "result_dir/label_test.txt")
